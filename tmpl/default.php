        ...
        <div class="contentbloc">
			<?php foreach ($params->get('add_customblock') as $block) : ?>
				<?php if ($block->TypofBlock != 'alb') : ?>
                    <div class="block <?php echo $block->nameblockcustom; ?> card"
                         style="width:<?php echo $block->width; ?>%">
                        <div class="card-header">
							<?php
							$catidslist    = [];
							$show_all_link = '';

							if (!empty($block->catidlist))
							{
							...
							$catidslist = empty($catidslist) ? '' : '&' . implode('&', $catidslist);

							switch ($block->TypofBlock)
							{
								case ('fb'):
									$show_all_link = Route::_('index.php?option=com_content&view=featured' . $catidslist);
									break;
								case ('pb'):
									$show_all_link = Route::_('index.php?option=com_content&view=articles&filter[published]=1' . $catidslist);
									break;
								case ('upb'):
									$show_all_link = Route::_('index.php?option=com_content&view=articles&filter[published]=0' . $catidslist);
									break;
								case ('ab'):
									$show_all_link = Route::_('index.php?option=com_content&view=articles&filter[published]=2' . $catidslist);
									break;
								case ('tb'):
									$show_all_link = Route::_('index.php?option=com_content&view=articles&filter[published]=-2' . $catidslist);
									break;
								case ('ub'):
								case ('cb'):
									$show_all_link = Route::_('index.php?option=com_content&view=articles&filter[author_id]=' . $user->id . $catidslist);
									break;
							}
							?>
                            <h3 class="module-title">
								<?php
								$icon  = !empty($block->iconbutton) ? $block->iconbutton : 'fa-star';
								$style = !empty($block->coloricon) ? 'style="color: ' . $block->coloricon . '"' : '';
								?>
                                <i class="fa <?php echo $icon; ?>" <?php echo $style; ?>></i>
								<?php echo $block->nameblockcustom; ?>
                            </h3>
							<?php if (!empty($show_all_link)): ?>
                                <div class="module-actions">
                                    <a href="<?php echo $show_all_link ?>" class="adminlink">
										<?php echo Text::_('MOD_DASHBOARD_ALL'); ?>
                                    </a>
                                </div>
							<?php endif; ?>
                        </div>
                        ...
                        <div class="card-body" style="height:<?php echo $block->forceheightblock; ?>">
							<?php if (count($actionlist)) : ?>
                                <table class="table">
                                    ...
                                    <tr>
                                        <th scope="col"
                                            class="w-80"><?php echo Text::_('MOD_LATESTACTIONS_ACTION'); ?></th>
                                        <th scope="col" class="w-20"><?php echo Text::_('JDATE'); ?></th>
                                    </tr>
                                    </thead>
                                    <tbody>
